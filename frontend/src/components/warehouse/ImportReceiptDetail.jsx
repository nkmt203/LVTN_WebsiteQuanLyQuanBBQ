  ...
  return (
    <div>
      <div className="grid grid-cols-2 gap-2 text-sm text-stone-600 mb-4">
        <p>Nhà cung cấp: <span className="font-medium text-stone-800">{phieuNhap.ten_nha_cung_cap}</span></p>
        <p>Người lập: <span className="font-medium text-stone-800">{phieuNhap.ten_nv_lap}</span></p>
        <p>Ngày nhập: <span className="font-medium text-stone-800">{new Date(phieuNhap.ngay_nhap).toLocaleString('vi-VN')}</span></p>
        <p>Tổng tiền: <span className="font-medium text-stone-800">{Number(phieuNhap.tong_tien).toLocaleString('vi-VN')}đ</span></p>
      </div>
      {phieuNhap.ghi_chu && <p className="text-sm text-stone-500 italic mb-3">Ghi chú: {phieuNhap.ghi_chu}</p>}
